...
export default function LineItemPicker({ onSelect, tipos = ['labor', 'product', 'fee'], catalogOptions = [], placeholder = 'Buscar en catálogo...' }) {
      ...
      {open && (
        <>
          ...
          <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, marginTop: 4, background: 'var(--surface)', border: '1.5px solid var(--border)', borderRadius: 8, boxShadow: '0 4px 20px rgba(0,0,0,0.12)', zIndex: 20, maxHeight: 300, overflowY: 'auto' }}>
            {results.length === 0 ? (
              <p style={{ padding: '10px 12px', fontSize: 12.5, color: 'var(--muted)' }}>Sin resultados.</p>
            ) : results.map(c => (
