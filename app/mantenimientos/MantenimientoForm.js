...
export default function MantenimientoForm({ editing, technicians, clients, clientProperties, onSaved, onCancel }) {
  const t = useTranslations('mantenimientos.form');
  ...
  const [technicianIds, setTechnicianIds] = useState(() => editing
    ? [editing.technician_id, ...(editing.recurring_maintenance_technicians ?? []).map(rmt => rmt.technician_id)].filter(Boolean)
    : []);
  ...
  return (
    <div className="card" style={{ marginBottom: 20, borderLeft: '4px solid var(--amber)' }}>
      <p style={{ fontWeight: 700, fontSize: 13, color: 'var(--navy)', marginBottom: 16 }}>{editing ? t('editTitle') : t('newTitle')}</p>
      ...
      <div style={{ display: 'grid', gap: 12, marginBottom: 16 }}>
            ...
            <select value={propertyId} onChange={e => selectProperty(e.target.value)} style={{ marginBottom: 6 }}>
              ...
              {clientProps.map(p => (
                <option key={p.id} value={p.id}>
                  {p.name ? `${p.name} — ` : ''}{[p.street, p.city].filter(Boolean).join(', ')}{p.is_primary ? ` ${t('primaryTag')}` : ''}
                </option>
              ))}
            </select>
          )}
        ...
        <p style={{ fontSize: 11.5, color: 'var(--muted)', marginTop: -6 }}>
          {frequency === 'weekly'
            ? t('repeatWeeklyHelp')
            : t('repeatOtherHelp', {
                unit: frequency === 'monthly' ? t('unit.monthly') : frequency === 'quarterly' ? t('unit.quarterly') : t('unit.yearly'),
                day: nextRunDate ? new Date(nextRunDate + 'T00:00:00').getDate() : '',
              })}
        </p>
        ...
        <div>
          <label style={{ fontSize: 13, fontWeight: 600, color: 'var(--navy)' }}>{t('checklistLabel')}</label>
          <div style={{ display: 'grid', gap: 6, marginTop: 6 }}>
            {items.map((item, i) => (
              <div key={i} style={{ display: 'flex', gap: 6 }}>
                <input value={item} onChange={e => updateItem(i, e.target.value)} style={{ flex: 1 }} placeholder={t('itemPlaceholder', { n: i + 1 })} />
                {items.length > 1 && (
                  <button type="button" onClick={() => removeItem(i)} className="btn btn-ghost" style={{ padding: '4px 10px' }}>×</button>
                )}
              </div>
            ))}
            <button type="button" onClick={addItem} className="btn btn-ghost" style={{ fontSize: 12, alignSelf: 'flex-start' }}>+ {t('addItem')}</button>
          </div>
        </div>
      </div>
      ...
      <div style={{ background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 8, padding: '10px 14px', fontSize: 13, color: 'var(--muted)', marginBottom: 16 }}>
        {t('autoTaskInfo', { multiple: technicianIds.length > 1 ? 'yes' : 'no' })}
      </div>
