...
export default async function FacturaDetail({ params }) {
  ...
  const [{ data: inv }, { data: items }, { data: payments }] = await Promise.all([
    supabase.from('invoices').select('*, clients(name, email, phone, company, client_type, client_addresses(*))').eq('id', id).single(),
    ...
    <div className="admin-shell">
      ...
      <main className="main-content">
        ...
        <div className="card" id="invoice-doc" style={{ marginBottom: 20 }}>
          ...
          {/* Bill to */}
          <div style={{ background: '#f8f9fb', borderRadius: 10, padding: '16px 20px', marginBottom: 28 }}>
            ...
            <div style={{ fontWeight: 700, fontSize: 16 }}>{inv.clients?.name}</div>
            {inv.clients?.company && <div style={{ color: 'var(--muted)', fontSize: 14 }}>{inv.clients.company}</div>}
            {primaryAddr && <div style={{ color: 'var(--muted)', fontSize: 13, marginTop: 4 }}>{primaryAddr.line1}, {primaryAddr.city} {primaryAddr.zip}</div>}
