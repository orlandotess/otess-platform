...
export default function InvoiceActions({ invoiceId, status, invoiceNumber, showPaymentOnly = false, balance = 0 }) {
  ...
  return (
    <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
      <button className="btn btn-ghost" onClick={printPDF}>🖨️ PDF</button>
        ...
        <form onSubmit={onSave}>
          <div className="form-row" style={{ marginBottom: 16 }}>
            <div className="form-group">
              <label>Monto *</label>
              <input type="number" value={payment.amount} onChange={e => set('amount', e.target.value)} step="0.01" min="0.01" max={balance} placeholder={`Máx $${Number(balance).toFixed(2)}`} required />
            </div>
            <div className="form-group">
              <label>Fecha</label>
          ...
          <div className="form-group" style={{ marginBottom: 16 }}>
            <label>Referencia (cheque #, confirmación, etc.)</label>
            <input value={payment.reference} onChange={e => set('reference', e.target.value)} placeholder="Opcional" />
          </div>
