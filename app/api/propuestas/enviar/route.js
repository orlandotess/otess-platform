...
export async function POST(request) {
  ...
  const t = await getClientEmailTranslator('emails.proposalSend');
    ...
    await getResend().emails.send({
      ...
      html: `
        <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:20px">
          ...
          <p style="font-size:14px;color:#444;line-height:1.6">${t('intro', { title: proposal.title, number: proposal.proposal_number })}</p>
          ...
          <p style="font-size:12px;color:#999">${t('footerQuestions')}</p>
        </div>
      `,
    });
  } catch (err) {
