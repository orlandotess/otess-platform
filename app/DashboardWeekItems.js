...
export default function DashboardWeekItems({ jobs, visits, events, tasks, absences, techColors, canQuickReschedule }) {
  const t = useTranslations('home.weekItems');
  ...
  const items = useMemo(() => {
    const list = [
      ...
      ...tasks.map(task => ({ kind: 'task', sortAt: task.due_at, data: task })),
  ...
  async function saveQuickReschedule(form) {
    ...
    setSaving(false);
    if (error) { alert(t('moveDateError', { message: error.message })); return; }
    setQuickReschedule(null);
    router.refresh();
  }

  if (items.length === 0) {
    return <div style={{ fontSize: 13, color: 'var(--muted)', padding: '12px 0' }}>{t('nothingScheduledThisWeek')}</div>;
  }
          ...
          if (kind === 'absence') {
            return (
              <div key={`a${data.id}`} style={{ fontSize: 12.5, fontWeight: 700, color: 'var(--warn)' }}>
                🚫 {t('technicianAbsent', { name: data.technicians?.name ?? t('technicianFallback') })} · {fmtDay(sortAt)}
              </div>
            );
          }
            ...
            <div key={`${kind}${data.id}`} onClick={() => openItem(kind, data)}
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                  ...
                  {badge && <span className={`badge ${badge}`} style={{ fontSize: 10, flexShrink: 0 }}>{statusKey ? t(`statusLabels.${statusKey}`) : data.status}</span>}
                </div>
