...
export default async function ClienteDetailPage({ params }) {
  ...
  const [{ data: client }, { data: jobs }, { data: invoices }, { data: properties }, { data: contacts }, { data: proposals }, { data: internalNotes }, { data: serviceTickets }] = await Promise.all([
    ...
    supabase.from('jobs').select('id, title, status, scheduled_start, property_id, property_name, street, city, state, zip, contact_id, technician_id, technicians(id, name), job_technicians(technician_id, technicians(name))').eq('client_id', id).order('scheduled_start', { ascending: false }),
