...
export default function OpportunityModal({ opp, stages, technicians, clients, onClose, onSaved, onDeleted, onClientCreated }) {
  const t = useTranslations('oportunidades.opportunityModal');
  const isNew = !opp.id;
  ...
  async function handleSave() {
    ...
    if (!form.stage_key) { setError(t('errors.stageRequired')); return; }
    setSaving(true);
    setError('');
  ...
  async function handleDelete() {
    if (!confirm(t('confirmDelete', { name: form.name }))) return;
    setSaving(true);
  ...
  async function handleConvertToClient() {
    ...
    if (!name) { setError(t('errors.convertNameRequired')); return; }
    setSaving(true);
    setError('');
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20 }}
      ...
      <div style={{ background: 'var(--surface)', borderRadius: 16, padding: 28, width: 520, maxWidth: '95vw', maxHeight: '90vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 18 }}>
          <div style={{ fontSize: 17, fontWeight: 800, color: 'var(--navy)' }}>{isNew ? t('titleNew') : t('titleEdit')}</div>
        ...
        <div className="form-group" style={{ marginBottom: 14 }}>
          ...
          <select value={form.client_id} onChange={e => pickClient(e.target.value)}>
            <option value="">{t('newProspectOption')}</option>
        ...
        <div className="form-row" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 14 }}>
          ...
          <div className="form-group">
            ...
            <input value={form.company_name} onChange={e => set('company_name', e.target.value)} placeholder={t('companyPlaceholder')} />
          </div>
        </div>
        ...
        <div className="form-row" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 14 }}>
          <div className="form-group">
            ...
            <input type="number" min="0" step="0.01" value={form.value} onChange={e => set('value', e.target.value)} placeholder={t('valuePlaceholder')} />
          </div>
          <div className="form-group">
        ...
        {canConvert && (
          <div style={{ background: 'var(--badge-green-bg)', border: '1px solid var(--ok)', borderRadius: 10, padding: 12, marginBottom: 16 }}>
            <div style={{ fontSize: 13, color: 'var(--navy)', marginBottom: 8 }}>{t('convertBanner')}</div>
