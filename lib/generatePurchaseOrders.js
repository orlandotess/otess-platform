...
export async function generatePurchaseOrders(items, { sourceType, sourceId, sourceLabel, client = supabase }) {
  ...
  const { data: last } = await client.from('purchase_orders').select('order_number').order('created_at', { ascending: false }).limit(1).maybeSingle();
  let nextNum = 1001;
  ...
  for (const [vendorName, vendorItems] of byVendor) {
    let { data: vendor } = await client.from('vendors').select('id').ilike('name', vendorName).maybeSingle();
    if (!vendor) {
      const { data: newVendor, error: vendorErr } = await client.from('vendors').insert([{ name: vendorName }]).select().single();
    ...
    const { error: itemsErr } = await client.from('purchase_order_items').insert(
      vendorItems.map(it => ({
