...
import { uploadJobPhoto } from './uploadJobPhoto';
...
export function useJobChecklist(jobId, initialItems, initialAreaPhotos = []) {
  ...
  async function duplicateGroup(groupKey) {
    ...
    for (const item of items) {
      const { data: newTop } = await supabase.from('job_checklist_items').insert([{
        ...
        completed: false,
        source_line_item_id: item.source_line_item_id ?? null,
        item_type: item.item_type ?? null,
      }]).select().single();
      if (!newTop) continue;
      newItems.push({ ...newTop, photo_signed_url: item.photo_signed_url ?? null, photo_signed_urls: item.photo_signed_urls ?? null });
      ...
      for (const child of children) {
        const { data: newChild } = await supabase.from('job_checklist_items').insert([{
          ...
          photo_urls: child.photo_urls ?? null,
          sort_order: child.sort_order,
          completed: false,
          source_line_item_id: child.source_line_item_id ?? null,
          item_type: child.item_type ?? null,
        }]).select().single();
        if (newChild) newItems.push({ ...newChild, photo_signed_url: child.photo_signed_url ?? null, photo_signed_urls: child.photo_signed_urls ?? null });
      }
    }
  ...
  async function handleAreaPhotoFile(e) {
    ...
    const { path: finalPath, error } = await uploadJobPhoto(path, file, { upsert: true });
    if (!error) {
      ...
      await supabase.from('job_checklist_areas').upsert(
        { job_id: jobId, group_name: groupName, photo_url: finalPath },
        { onConflict: 'job_id,group_name' }
      );
      setAreaPhotos(prev => ({ ...prev, [groupName]: { photo_url: finalPath, photo_signed_url: signed?.signedUrl ?? null } }));
    }
  ...
  async function duplicateItem(item) {
    ...
    if (!data) return;
    const newItems = [{ ...data, photo_signed_url: item.photo_signed_url ?? null, photo_signed_urls: item.photo_signed_urls ?? null }];
    // Only top-level items carry children — a sub-item never becomes a parent (see file header).
  ...
  async function handleItemPhotoFile(e) {
    ...
    e.target.value = '';
    if (!files.length || !pendingPhotoItemId) return;
    ...
    for (let idx = 0; idx < files.length; idx++) {
      ...
      const ext = file.name.split('.').pop();
      const path = `${jobId}/checklist-${itemId}-${Date.now()}-${idx}.${ext}`;
      const { path: finalPath, error } = await uploadJobPhoto(path, file, { upsert: true });
    ...
    if (uploadedPaths.length) {
      ...
      const existing = checklistItems.find(i => i.id === itemId);
      const existingPaths = existing?.photo_urls ?? (existing?.photo_url ? [existing.photo_url] : []);
      const existingSigned = existing?.photo_signed_urls ?? (existing?.photo_signed_url ? [existing.photo_signed_url] : []);
      const mergedPaths = [...existingPaths, ...uploadedPaths];
      const mergedSigned = [...existingSigned, ...signedUrls.filter(Boolean)];
      await supabase.from('job_checklist_items').update({ photo_url: mergedPaths[0], photo_urls: mergedPaths }).eq('id', itemId);
      setChecklistItems(prev => prev.map(i => i.id === itemId
        ? { ...i, photo_url: mergedPaths[0], photo_urls: mergedPaths, photo_signed_url: mergedSigned[0] ?? null, photo_signed_urls: mergedSigned }
        : i));
    }
  ...
  async function removeItemPhoto(itemId) {
    setItemMenuOpen(null);
    await supabase.from('job_checklist_items').update({ photo_url: null, photo_urls: null }).eq('id', itemId);
    setChecklistItems(prev => prev.map(i => i.id === itemId ? { ...i, photo_url: null, photo_urls: null, photo_signed_url: null, photo_signed_urls: null } : i));
  }
