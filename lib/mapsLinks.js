...
// Single best link for a one-tap "open GPS" action — Apple Maps on iOS
// (where it hands off to the native app), Google Maps everywhere else.
export function pickMapsLink(street, city, state, zip) {
  const links = buildMapsLinks(street, city, state, zip);
  if (links.direct) return links.direct;
